...
export const CuttingScreen = ({ navigation }: RootStackScreenProps<'Cutting'>) => {
  ...
  const { data: jobs, isLoading: jobsLoading, error: jobsError } = useQuery<CuttingJob[]>({
    queryKey: ['/api/cutting-jobs'],
    queryFn: async () => {
      const response = await fetchWithRetry(getFullApiUrl(API_ENDPOINTS.cuttingJobs));
      return response;
    },
  });
  ...
  }, [jobs, blocks, searchTerm, sortField, sortOrder]);
  ...
  const renderJobCard = ({ item }: { item: CuttingJob }) => {
          ...
          Alert.alert(
            ...
            [
              ...
              { 
                text: 'Edit',
                onPress: () => {
                  navigation.navigate('EditCuttingJob', { jobId: item.id });
                },
              },
            ]
          );
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('NewCuttingJob')}
      >
        <Text style={styles.addButtonText}>+ New Cutting Job</Text>
      </TouchableOpacity>
    </View>
  );
};
